<script>
    ...
    import {AudioLines, CirclePause, Menu, RadioTower, Search, SkipBack, SkipForward} from 'lucide-svelte';
    ...
    let nowPlayingImageColor = undefined;
    ...
    onMount(() => {
        ...
        window.onSpotifyWebPlaybackSDKReady = async function () {
            ...
            const nowPlaying = async () => {
             ...
             nowPlaying().then(r => {
                 nowPlayingObject = r.data;
                 nowPlayingImageUrl = nowPlayingObject.item.album.images[0].url;
                 console.log(nowPlayingObject);
...
<main style={`background: linear-gradient(0deg, #171313 0%, ${nowPlayingImageColor || '#181414'} 100%);`} class="h-screen grid grid-cols-11 grid-rows-12">
    ...
    <div class="col-start-3 col-end-10 row-start-9 row-end-11 lg:pt-5">
        {#if nowPlayingObject}
            <h3 class="text-2xl text-white p-0">{nowPlayingObject.item.name}</h3>
            <h4 class="text-xl text-gray-600 p-0">{nowPlayingObject.item.artists.map(artist => artist.name).join(', ')}</h4>
        {/if}
    </div>
